...
async function create_children (children, values, component) {
    const results         = [];
    const occurrences     = [];
    ...
    for (let [index, value] of values.entries()) {
        ...
        if (child_index >= 0) {
            ...
            results.push(children[child_index]);
            continue;
        }

        results.push(
            await create_new_child(value, index, component)
        );
    }
...
module.exports = {
    ...
    controller : {
        ...
        async on_digest () {
            ...
            component.children = await create_children(
